...
NGRAM_GCN_INPUT_FASTA_PATH = "C:/ProgramData/ProtDiGCN/uniprot_sprot.fasta"
...
def extract_canonical_id_and_type(header_or_id_line: str) -> tuple[Optional[str], Optional[str]]:
    ...
    if pdb_match:
        ...
        if not is_likely_uniprot:
            return "PDB", f"{pdb_id}{'_' + chain_part if chain_part else ''}"
...
def get_ngrams_and_transitions_ngram(sequences: list[str], n: int):
    ...
    for seq in sequences:
        ...
        for i in range(len(current_seq_ngrams) - 1):
            all_transitions.append((current_seq_ngrams[i], current_seq_ngrams[i + 1]))
...
def build_ngram_graph_data_ngram(ngrams: list[tuple], transitions: list[tuple[tuple, tuple]], node_prob_from_prev_graph: Optional[dict[tuple, float]] = None, n_val: int = 1) -> tuple[
    ...
    if hasattr(data, 'edge_index_out') and data.edge_index_out is not None and data.edge_index_out.numel() > 0:
        for i in range(data.edge_index_out.size(1)):
            training_samples.append((data.edge_index_out[0, i].item(), data.edge_index_out[1, i].item()))
    data.training_samples = training_samples
    return data, ngram_to_idx, idx_to_ngram, ngram_probabilities


def get_sinusoidal_positional_embeddings_ngram(max_len: int, d_model: int) -> torch.Tensor:
    pe = torch.zeros(max_len, d_model)
    position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
    div_term_exp = torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model)
    div_term = torch.exp(div_term_exp)
    pe[:, 0::2] = torch.sin(position * div_term)
    if d_model % 2 != 0:
        if d_model == 1:
            pass
        elif div_term.size(0) > 0:
            pe[:, 1::2] = torch.cos(position * div_term[:((d_model - 1) // 2) + 1])
    else:
        if div_term.size(0) > 0:
...
class CustomDiGCNLayerPyG_ngram(MessagePassing):
    ...
    def forward(self, x: torch.Tensor, edge_index_in: torch.Tensor, edge_weight_in: torch.Tensor, edge_index_out: torch.Tensor, edge_weight_out: torch.Tensor):
        ...
        if self.use_vector_coeffs:
            C_in_vec_, C_out_vec_ = self.C_in_vec.to(x.device), self.C_out_vec.to(x.device)
            if C_in_vec_.size(0) != x.size(0): raise ValueError(f"NgramGCN: C_in_vec num_nodes ({C_in_vec_.size(0)}) mismatch with input x ({x.size(0)})")
...
# ... (imports, CustomDiGCNLayerPyG_ngram class, _apply_positional_encoding, and __init__ method of ProtDiGCNEncoderDecoder_ngram
# remain the same as the version that introduced residual connections and projection layers) ...

class ProtDiGCNEncoderDecoder_ngram(nn.Module):
    def __init__(self, num_initial_features: int, hidden_dim1: int, hidden_dim2: int, num_nodes_in_graph: int,
                 n_gram_length_for_pe: int, one_gram_embed_dim_for_pe: int, max_allowable_ngram_len_for_pe: int,
        ...
        self.l2_norm_eps = 1e-12  # Epsilon for L2 normalization

        self.positional_encoder = None
        if self.one_gram_embed_dim_for_pe > 0 and max_allowable_ngram_len_for_pe > 0:
            self.positional_encoder = get_sinusoidal_positional_embeddings_ngram(max_allowable_ngram_len_for_pe, self.one_gram_embed_dim_for_pe)

        # GCN Layers
        self.conv1 = CustomDiGCNLayerPyG_ngram(num_initial_features, hidden_dim1, num_nodes_in_graph, use_vector_coeffs_in_gnn)
        self.conv2 = CustomDiGCNLayerPyG_ngram(hidden_dim1, hidden_dim1, num_nodes_in_graph, use_vector_coeffs_in_gnn)
        self.conv3 = CustomDiGCNLayerPyG_ngram(hidden_dim1, hidden_dim2, num_nodes_in_graph, use_vector_coeffs_in_gnn)
        ...
        # This attribute will store the L2-normalized embeddings intended for extraction
        self.final_normalized_embedding_output = None

    def _apply_positional_encoding(self, x: torch.Tensor) -> torch.Tensor:
        # ... (this method remains unchanged)
        if self.positional_encoder is None or self.n_gram_length_for_pe == 0 or self.one_gram_embed_dim_for_pe == 0: return x
        if self.n_gram_length_for_pe == 1:
            if x.shape[1] == self.one_gram_embed_dim_for_pe: x = x + self.positional_encoder[0, :].to(x.device).unsqueeze(0)
        elif self.n_gram_length_for_pe > 1:
            expected_dim = self.n_gram_length_for_pe * self.one_gram_embed_dim_for_pe
            if x.shape[1] == expected_dim:
                ...
                if pe_len_to_use < self.n_gram_length_for_pe:
                    x_with_pe_reshaped = torch.cat((x_reshaped_pe_part, x_reshaped[:, pe_len_to_use:, :]), dim=1)
                else:
                    x_with_pe_reshaped = x_reshaped_pe_part
                x = x_with_pe_reshaped.view(-1, expected_dim)
        return x

    def forward(self, data: Data) -> tuple[torch.Tensor, torch.Tensor]:
        current_device = data.x.device if hasattr(data, 'x') and data.x is not None else (data.edge_index_in.device if hasattr(data, 'edge_index_in') and data.edge_index_in is not None else (
            data.edge_index_out.device if hasattr(data, 'edge_index_out') and data.edge_index_out is not None else 'cpu'))
        ...
        if x_on_device.numel() == 0 and data.num_nodes > 0 and hasattr(self.conv1, 'lin_main_in') and self.conv1.lin_main_in.in_features > 0:
            if DEBUG_VERBOSE: print(f"NgramGCN Model: Warning - data.x is empty for {data.num_nodes} nodes. Using random features of dim {self.conv1.lin_main_in.in_features}.")
        ...
        if x_pe.numel() == 0 and data.num_nodes > 0:
            print(f"NgramGCN Model Error: Input features 'x' are empty for a graph with {data.num_nodes} nodes after PE. Cannot proceed.")
            empty_logits = torch.empty((data.num_nodes, self.num_nodes_in_graph), device=current_device)
            empty_embeds_dim = self.conv3.lin_main_in.out_features if hasattr(self.conv3, 'lin_main_in') else self.num_nodes_in_graph
            empty_embeds = torch.empty((data.num_nodes, empty_embeds_dim), device=current_device)
            return empty_logits, empty_embeds

        # --- Block 1 (conv1 + residual + tanh + dropout) ---
        ...
        # --- Block 2 (conv2 + residual + tanh + dropout) ---
        ...
        # This is the final GCN output after activation, used for the decoder path (pre-normalization)
        final_gcn_activated_output = F.tanh(h3_res_sum)

        # --- Decoder Path (uses Tanh-activated but pre-L2 normalization embeddings) ---
        h_embed_for_decoder_dropped = F.dropout(final_gcn_activated_output, p=self.dropout_rate, training=self.training)
        next_hop_logits = self.decoder_fc(h_embed_for_decoder_dropped)

        # --- L2 Normalization for EXTRACTED Embeddings ---
        # The L2 normalized version of final_gcn_activated_output is what we return for extraction.
        ...
        # Return log_softmax for loss, and the L2-NORMALIZED embeddings for extraction
        return F.log_softmax(next_hop_logits, dim=-1), self.final_normalized_embedding_output


def train_ngram_model_ngram(model: ProtDiGCNEncoderDecoder_ngram, data: Data, optimizer: optim.Optimizer, epochs: int, device: torch.device):
    model.train();
    model.to(device);
    data = data.to(device)

    if not hasattr(data, 'training_samples') or not data.training_samples:
        if DEBUG_VERBOSE: print("NgramGCN: No training_samples in data. Cannot train."); return
    ...
    if source_indices.numel() == 0:
        if DEBUG_VERBOSE: print("NgramGCN: Source indices for training empty. Cannot train."); return
    ...
    for epoch in range(1, epochs + 1):
        ...
        if log_probs.numel() == 0:
            if DEBUG_VERBOSE: print(f"NgramGCN: Epoch {epoch}: Model returned empty logits. Skipping loss calculation.")
            continue
        if source_indices.max() >= log_probs.size(0):
            print(f"NgramGCN: Error: Max source index {source_indices.max()} OOB for log_probs {log_probs.size(0)}")
            return
        if target_indices.max() >= log_probs.size(1) or target_indices.min() < 0:
            print(f"NgramGCN: Error: Target index OOB. Max: {target_indices.max()}, Min: {target_indices.min()} vs Decoder Output Size: {log_probs.size(1)}")
            return
        loss = criterion(log_probs[source_indices], target_indices)
        loss.backward();
        optimizer.step()
        if epoch % (max(1, epochs // 10 if epochs >= 10 else 1)) == 0 or epoch == epochs:
            if DEBUG_VERBOSE: print(f"NgramGCN: Epoch: {epoch:03d}, Loss: {loss.item():.4f}")
...
def extract_node_embeddings_ngram(model: ProtDiGCNEncoderDecoder_ngram, data: Data, device: torch.device) -> Optional[np.ndarray]:
    ...
    model.to(device);
    data = data.to(device)
    if not hasattr(data, 'x') or data.x is None or (data.x.numel() == 0 and data.num_nodes > 0):
        if DEBUG_VERBOSE: print("NgramGCN Extract: data.x is missing or empty for a non-empty graph. Cannot extract embeddings.")
        return np.array([])
    with torch.no_grad():
...
def generate_and_save_ngram_embeddings(fasta_filepath: str, protein_ids_to_generate_for: Set[str], max_n_for_ngram: int, output_dir_for_this_run: str, one_gram_init_embed_dim: int, hidden_dim1: int, hidden_dim2: int,
                                       pe_max_len: int, dropout: float, lr: float, epochs_per_level: int, use_vector_gnn_coeffs: bool, ngram_l2_reg: float) -> Tuple[Optional[str], Optional[str]]:
    ...
    if not all_protein_data_from_fasta:
        print("NgramGCN: No sequences parsed from NgramGCN input FASTA. Cannot generate N-gram embeddings.")
        return None, None

    sequences_to_process_map = {parsed_pid: seq for parsed_pid, seq in all_protein_data_from_fasta if parsed_pid in protein_ids_to_generate_for}

    if not sequences_to_process_map:
        print(
            f"NgramGCN: No sequences from '{os.path.basename(fasta_filepath)}' match the 'protein_ids_to_generate_for' set ({len(protein_ids_to_generate_for)} IDs). Cannot generate N-gram embeddings for this evaluation.")
        return None, None
    ...
    if DEBUG_VERBOSE: print(f"NgramGCN: Will process {len(sequences_list_for_ngram)} sequences for N-gram generation (using standardized IDs).")
    ...
    for n_val in range(1, max_n_for_ngram + 1):
        ...
        if not current_ngrams:
            print(f"NgramGCN: No {n_val}-grams found for the filtered sequences. Stopping generation for this N-gram level.")
            break
        prev_n_gram_probs = level_node_probabilities.get(n_val - 1) if n_val > 1 else None
        graph_data, ngram_to_idx_map, idx_to_ngram_map, ngram_probs = build_ngram_graph_data_ngram(current_ngrams, current_transitions, prev_n_gram_probs, n_val)

        if graph_data is None or graph_data.num_nodes == 0:
            print(f"NgramGCN: Could not build graph or graph has no nodes for n={n_val}. Stopping generation at this level.")
            break
        ...
        if n_val == 1:
            ...
            if one_gram_embeds_arr is None or one_gram_char_tuple_to_idx_map is None or one_gram_embeds_arr.size == 0:
                print(f"NgramGCN: Error: 1-gram base data missing or empty for n={n_val} features. Stopping generation.")
                break
            features_list = []
            current_lvl_idx_to_ngram = level_idx_to_ngram[n_val]
            expected_concat_dim = n_val * one_gram_init_embed_dim

            for i in range(graph_data.num_nodes):
                current_multichar_ngram = current_lvl_idx_to_ngram.get(i)
                if current_multichar_ngram is None:
                    features_list.append(torch.zeros(expected_concat_dim));
                    continue
                feature_parts = []
                for residue_char in current_multichar_ngram:
                    residue_1gram_tuple_key = (residue_char,)
                    if residue_1gram_tuple_key in one_gram_char_tuple_to_idx_map:
                        idx_in_1gram = one_gram_char_tuple_to_idx_map[residue_1gram_tuple_key]
                        if idx_in_1gram < len(one_gram_embeds_arr):
                            feature_parts.append(torch.from_numpy(one_gram_embeds_arr[idx_in_1gram].copy()).float())
                        else:
                            feature_parts.append(torch.zeros(one_gram_init_embed_dim))
                    else:
                        feature_parts.append(torch.zeros(one_gram_init_embed_dim))

                concatenated_feature = torch.cat(feature_parts) if feature_parts else torch.zeros(expected_concat_dim)
                if concatenated_feature.shape[0] != expected_concat_dim:
                    if DEBUG_VERBOSE: print(f"NgramGCN Warning: Feature dim mismatch for ngram {current_multichar_ngram}. Got {concatenated_feature.shape[0]}, expected {expected_concat_dim}. Padding/truncating.")
                    if concatenated_feature.shape[0] < expected_concat_dim:
                        padding = torch.zeros(expected_concat_dim - concatenated_feature.shape[0])
                        concatenated_feature = torch.cat((concatenated_feature, padding))
                    else:
                        concatenated_feature = concatenated_feature[:expected_concat_dim]
                features_list.append(concatenated_feature)

            if not features_list and graph_data.num_nodes > 0:
                print(f"NgramGCN: Error: No features created for n={n_val} despite having nodes. Stopping.");
                break

            graph_data.x = torch.stack(features_list) if features_list else torch.empty((0, expected_concat_dim))
            current_feature_dim = graph_data.x.shape[1] if graph_data.x.numel() > 0 else 0
            if DEBUG_VERBOSE: print(f"NgramGCN: Initialized n={n_val} features (new dim: {current_feature_dim}).")

        if graph_data.num_nodes > 0 and current_feature_dim > 0:
            if not (hasattr(graph_data, 'edge_index_out') and graph_data.edge_index_out is not None and graph_data.edge_index_out.numel() > 0):
                if DEBUG_VERBOSE: print(f"NgramGCN: No edges in graph for n={n_val}. Skipping model training as training_samples would be empty.")
                level_embeddings[n_val] = np.array([])
                if n_val == 1:
                    print("NgramGCN: 1-gram graph has no edges. Cannot proceed to higher N-grams that depend on 1-gram embeddings.")
                    break
                continue

            model = ProtDiGCNEncoderDecoder_ngram(current_feature_dim, hidden_dim1, hidden_dim2, graph_data.num_nodes, n_val, one_gram_init_embed_dim, pe_max_len, dropout, use_vector_gnn_coeffs)

            # --- ADD THIS SECTION TO PRINT MODEL ARCHITECTURE ---
            if DEBUG_VERBOSE and n_val == 1:  # Print for the first n-gram level if debugging
                print(f"\nNgramGCN Model Architecture (for n={n_val}):")
                print(model)
                print("-" * 50)  # Separator
            # --- END OF SECTION TO PRINT MODEL ARCHITECTURE ---

            optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=ngram_l2_reg)  # <-- MODIFIED THIS LINE
            if DEBUG_VERBOSE: print(f"NgramGCN: Training model for n={n_val}...")
            train_ngram_model_ngram(model, graph_data, optimizer, epochs_per_level, device)
            ...
            if node_embeddings_np is None or node_embeddings_np.size == 0:
                print(f"NgramGCN: Failed to extract valid embeddings for n={n_val}. Stopping generation.");
                break
            ...
            if n_val == 1:
                with h5py.File(per_residue_emb_path, 'w') as hf:
                    for idx, ngram_tuple in level_idx_to_ngram[1].items():
                        hf.create_dataset(ngram_tuple[0], data=level_embeddings[1][idx])
                print(f"NgramGCN: Saved per-residue (1-gram) embeddings to {per_residue_emb_path}")
        elif graph_data.num_nodes == 0:
            print(f"NgramGCN: No nodes in graph for n={n_val}. Skip model.")
        else:
            print(f"NgramGCN: Node features zero-dim for n={n_val}. Skip model.")

    if max_n_for_ngram not in level_embeddings or level_embeddings.get(max_n_for_ngram, np.array([])).size == 0 or max_n_for_ngram not in level_idx_to_ngram:
        print(f"NgramGCN: Embeddings for final n={max_n_for_ngram} not available or empty. Cannot gen per-protein embeds.")
        return None, (per_residue_emb_path if 1 in level_embeddings and level_embeddings.get(1, np.array([])).size > 0 and os.path.exists(per_residue_emb_path) else None)
    else:
        if DEBUG_VERBOSE: print(f"\nNgramGCN: --- Generating Per-Protein Embeddings (from n={max_n_for_ngram} graph) ---")
        final_n_gram_embeds = level_embeddings[max_n_for_ngram]
        final_n_gram_to_idx_map = level_ngram_to_idx[max_n_for_ngram]
        with h5py.File(per_protein_emb_path, 'w') as hf_protein:
            protein_count = 0
            for protein_id in tqdm(protein_ids_for_final_pooling, desc="NgramGCN: Pooling protein embeddings", disable=not DEBUG_VERBOSE):
                protein_sequence = sequences_to_process_map[protein_id]
                if len(protein_sequence) < max_n_for_ngram: continue
                ...
                for i in range(len(protein_sequence) - max_n_for_ngram + 1):
                    ...
                    if ngram_tuple in final_n_gram_to_idx_map:
                        ...
                        if ngram_idx < len(final_n_gram_embeds):
                            protein_specific_ngram_embeds.append(final_n_gram_embeds[ngram_idx])
                if protein_specific_ngram_embeds:
                    hf_protein.create_dataset(protein_id, data=np.mean(protein_specific_ngram_embeds, axis=0))
                    protein_count += 1
            print(f"NgramGCN: Saved {protein_count} per-protein embeddings (keyed by UniProt Accession) to {per_protein_emb_path}")

    res_path = per_residue_emb_path if 1 in level_embeddings and level_embeddings.get(1, np.array([])).size > 0 and os.path.exists(per_residue_emb_path) else None
    prot_path = per_protein_emb_path if max_n_for_ngram in level_embeddings and level_embeddings.get(max_n_for_ngram, np.array([])).size > 0 and os.path.exists(per_protein_emb_path) else None
    return prot_path, res_path


# ============================================================================
# END: N-gramGCN Code Integration
# ============================================================================

# ============================================================================
# START: Link Prediction Evaluater Code
# ============================================================================
...
class FileOps:
    ...
    @staticmethod
    def load_custom_embeddings(p_path: str, r_ids: Optional[Set[str]] = None) -> Dict[str, np.ndarray]:
...
def main_workflow_cv_lp(embedding_name_cv: str, protein_embeddings_cv: Dict[str, np.ndarray], positive_pairs_cv: List[Tuple[str, str, int]], negative_pairs_cv: List[Tuple[str, str, int]],
                        ...
                        max_shuff_buffer_cv: int, batch_size_cv: int, epochs_cv: int, learning_rate_cv: float, k_vals_ranking_cv: List[int]) -> Dict[str, Any]:
        ...
        if not ngram_fasta_path or not os.path.exists(ngram_fasta_path) or not ngram_output_base:
            ...
            if not all_ids_in_ngram_fasta_parsed:
                ...
                generated_prot_h5, _ = generate_and_save_ngram_embeddings(fasta_filepath=ngram_fasta_path, protein_ids_to_generate_for=all_ids_in_ngram_fasta_parsed,  # Generate for ALL in its FASTA
                                                                          ...
                                                                          one_gram_init_embed_dim=ngram_gcn_config.get('init_dim'), hidden_dim1=ngram_gcn_config.get('hidden_dim1'),
                                                                          hidden_dim2=ngram_gcn_config.get('hidden_dim2'), pe_max_len=ngram_gcn_config.get('pe_max_len'), dropout=ngram_gcn_config.get('dropout'),
                                                                          ...
                                                                          ngram_l2_reg=ngram_gcn_config.get('weight_decay', 0.0))
    ...
    for cfg_item_lp in processed_emb_configs_final_lp:
        ...
        if prot_embs_lp and len(prot_embs_lp) > 0:
            ...
            cv_res = main_workflow_cv_lp(embedding_name_cv=cfg_item_lp['name'], protein_embeddings_cv=prot_embs_lp, positive_pairs_cv=final_pos_pairs, negative_pairs_cv=final_neg_pairs,
                                         ...
                                         max_train_samp_cv_lp=lp_max_train_samples_cv, max_val_samp_cv_lp=lp_max_val_samples_cv, max_shuff_buffer_cv=lp_max_shuffle_buffer_size, batch_size_cv=lp_batch_size,
                                         epochs_cv=lp_epochs, learning_rate_cv=lp_learning_rate, k_vals_ranking_cv=lp_k_vals_ranking)
            if cv_res:
