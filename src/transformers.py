...
MODEL_CONFIGS = [
    ...
    {"name": "ProtT5_XL", "hf_id": "Rostlab/prot_t5_xl_uniref50", "is_t5": True, "batch_size_multiplier": 0.25} # T5 XL is large, use smaller batch
]
...
def simple_fasta_parser(fasta_filepath: str) -> Iterator[Tuple[str, str]]: # Copied from Script A
    current_id_full_header = None; uniprot_id = None; sequence_parts = []
    try:
        with open(fasta_filepath, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if line.startswith('>'):
                    if current_id_full_header and sequence_parts: yield uniprot_id if uniprot_id else current_id_full_header, "".join(sequence_parts)
                    ...
                    sequence_parts = []
                elif current_id_full_header: sequence_parts.append(line.upper())
            if current_id_full_header and sequence_parts: yield uniprot_id if uniprot_id else current_id_full_header, "".join(sequence_parts)
    except FileNotFoundError: print(f"Warning: FASTA file not found: {fasta_filepath}")
    except Exception as e: print(f"Warning: Error parsing FASTA file {fasta_filepath}: {e}")
...
def generate_embeddings_for_model(
    ...
    try:
        ...
        tokenizer = T5Tokenizer.from_pretrained(huggingface_model_id) if is_t5 else AutoTokenizer.from_pretrained(huggingface_model_id)
        ...
        if ENABLE_XLA:
            ...
            def compiled_model_call(inputs_dict_tf):
                if is_t5:
                    ...
                    decoder_input_ids_tf = tf.fill((num_seqs, 1), tf.cast(decoder_start_id, inputs_dict_tf['input_ids'].dtype))
                    return model(input_ids=inputs_dict_tf['input_ids'], attention_mask=inputs_dict_tf['attention_mask'], decoder_input_ids=decoder_input_ids_tf)
                else:
    ...
    for fasta_file_path in tqdm(fasta_files, desc=f"Files for {model_name_tag}"):
                ...
                try:
                    ...
                    for i in range(len(identifiers_batch)):
                        ...
                        if is_t5: # T5 usually doesn't have CLS, ends with EOS
                             ...
                             embeddings_to_save = raw_embeddings_batch_np[i, 1:original_seq_len+1, :] # Heuristic: skip CLS

                        if embeddings_to_save.shape[0] == original_seq_len : # Check if length matches
                             all_per_residue_embeddings_for_h5[seq_id] = embeddings_to_save.astype(np.float16) # Save as float16 for space
                        else:
                             # Fallback or warning if slicing heuristic is off
                             print(f"Warning: Length mismatch for {seq_id}. Original: {original_seq_len}, Emb shape[0]: {embeddings_to_save.shape[0]}. Using available embeddings up to original_seq_len.")
                             ...
                             all_per_residue_embeddings_for_h5[seq_id] = raw_embeddings_batch_np[i, :safe_len, :].astype(np.float16)
        ...
        if sequences_batch_text:
            try:
                inputs_tf = tokenizer(sequences_batch_text, padding="max_length", truncation=True, return_tensors="tf", max_length=max_seq_len)
                model_outputs = model_inference_func(inputs_tf)
                raw_embeddings_batch_tf = model_outputs.encoder_last_hidden_state if is_t5 else model_outputs.last_hidden_state
                raw_embeddings_batch_np = raw_embeddings_batch_tf.numpy()
                attention_mask_batch_np = inputs_tf['attention_mask'].numpy()
                if native_embedding_dim is None and raw_embeddings_batch_np.ndim == 3: native_embedding_dim = raw_embeddings_batch_np.shape[2]
                for i in range(len(identifiers_batch)):
                    seq_id = identifiers_batch[i]; original_seq_len = len(sequences_batch_text[i])
                    embeddings_to_save = raw_embeddings_batch_np[i, 1:original_seq_len+1, :] if not is_t5 else raw_embeddings_batch_np[i, :original_seq_len, :]
                    if embeddings_to_save.shape[0] == original_seq_len : all_per_residue_embeddings_for_h5[seq_id] = embeddings_to_save.astype(np.float16)
                    else: 
                        safe_len = min(original_seq_len, raw_embeddings_batch_np.shape[1])
                        all_per_residue_embeddings_for_h5[seq_id] = raw_embeddings_batch_np[i, :safe_len, :].astype(np.float16)
