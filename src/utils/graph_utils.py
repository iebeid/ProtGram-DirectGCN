...
# VERSION: 6.3 (Cleaned up debug prints, retained essential warnings)
# AUTHOR: Islam Ebeid
# ==============================================================================
...
class Graph:
    ...
    def _process_constructor_inputs(self):
        ...
        """
        # print("  [DEBUG Graph._process_constructor_inputs] Starting processing...") # Optional: Keep if a high-level marker is desired

        if not self.idx_to_node_map_from_constructor and not self.original_edges:
            self.number_of_nodes = 0
            self.edges = []
            self.number_of_edges = 0
            # print("  [Graph Constructor Info] No nodes or edges from constructor. Initializing empty graph.") # Optional
            return
        ...
        if not all_integer_indices:
            ...
            self.edges = []
            self.number_of_edges = 0
            # print("  [Graph Constructor Info] all_integer_indices is empty. Setting num_nodes to 0.") # Optional
            return
        ...
        # print(f"  [Graph Constructor Info] max_idx from all_integer_indices: {max_idx}, self.number_of_nodes SET TO: {self.number_of_nodes}") # Optional
        ...
        # Optional: Keep a summary print if desired, or remove if too verbose
        # print(
        #     f"  [Graph Constructor Info] Finished processing edges. Passed: {edge_pass_count}. Failed s_type: {edge_fail_s_idx_type_count}, s_bound: {edge_fail_s_idx_bound_count}, t_type: {edge_fail_t_idx_type_count}, t_bound: {edge_fail_t_idx_bound_count}")
        # print(f"  [Graph Constructor Info] Final self.edges count: {len(self.edges)}, self.number_of_edges: {self.number_of_edges}")
...
class DirectedNgramGraph(Graph):
    ...
    def _create_raw_weighted_adj_matrices_torch(self):
        if self.number_of_nodes == 0:
            self.A_out_w = torch.empty((0, 0), dtype=torch.float32)
            self.A_in_w = torch.empty((0, 0), dtype=torch.float32)
            # print("  [DirectedNgramGraph Info] No nodes, adj matrices are empty.") # Optional
            return

        self.A_out_w = torch.zeros((self.number_of_nodes, self.number_of_nodes), dtype=torch.float32)
        # print(f"  [DirectedNgramGraph Info] Initializing A_out_w for {self.number_of_nodes} nodes. Processing {len(self.edges)} edges.") # Optional
        ...
        for edge_tuple in self.edges:
            ...
            if 0 <= s_idx < self.number_of_nodes and 0 <= t_idx < self.number_of_nodes:
                self.A_out_w[s_idx, t_idx] = weight
            else:
                print(f"    [DirectedNgramGraph WARNING]: Edge ({s_idx},{t_idx}) out of bounds for num_nodes={self.number_of_nodes} during adj matrix creation. Skipping.")
        self.A_in_w = self.A_out_w.t().contiguous()
        # print(f"  [DirectedNgramGraph Info] A_out_w non-zero elements: {torch.count_nonzero(self.A_out_w)}") # Optional

    def _calculate_single_propagation_matrix_for_gcn(self, A_w_torch: torch.Tensor) -> np.ndarray:
        if self.number_of_nodes == 0 or A_w_torch.shape[0] == 0:
    ...
    def get_fai_sparse(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.fai is None or self.fai.numel() == 0 and self.number_of_nodes > 0:
            ...
            device_to_use = self.A_out_w.device if hasattr(self, 'A_out_w') and self.A_out_w.numel() > 0 else 'cpu'
