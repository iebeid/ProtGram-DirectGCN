...
class DummyDataFactory:
    ...
    def create_dummy_id_mapping_parquet(directory: str, filename: str = "dummy_id_mapping.parquet", num_ids: int = 5):
        ...
        # The real mapping file has 'From' and 'To' columns. We mimic that.
        # The 'From' IDs should match the UniProt accession numbers extracted
        # from the headers generated by create_fasta (e.g., DUMMY0001).
        ids = [f"DUMMY{i+1:04d}" for i in range(num_ids)]
        df = pd.DataFrame({
            'From': ids,
            'To': ids  # Simple 1-to-1 mapping is sufficient for the test
        })
